...
The Ltecoin Core daemon can be configured to act as a trusted "border
router", implementing the Ltecoin wire protocol and relay, making
...
The ZeroMQ feature in Ltecoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building Ltecoind:
...
For instance:
    ...
    $ Ltecoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/Ltecoind.tx.raw \
...
These options can also be provided in Ltecoin.conf.
...
From the perspective of Ltecoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into Ltecoind directly. Furthermore, no information is
...
using. Ltecoind appends an up-counting sequence number to each
