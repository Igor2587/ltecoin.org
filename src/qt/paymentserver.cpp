...
const QString BITCOIN_IPC_PREFIX("Ltecoin:");
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start Ltecoin: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("Ltecoin://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'Ltecoin://' is not a valid URI. Use 'Ltecoin:' instead."),
    ...
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Ltecoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
